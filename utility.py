...
class Datasets():
    def __init__(self, conf):
        ...
        self.graphs = [u_b_graph_train, u_i_graph, b_i_graph, new_ui_graph]

        self.train_loader = DataLoader(self.bundle_train_data, batch_size=batch_size_train, shuffle=True, num_workers=20, drop_last=True)
    ...
    def get_ui(self):
        ...
        plus = np.load(self.path+'/'+self.name+ self.enhance ,allow_pickle=True).tolist()

        index_new_x = indice[:, 0].tolist()
        index_new_y = indice[:, 1].tolist()
        ...
        return u_i_pairs, u_i_graph, new_ui_graph
