...
class EBRec(nn.Module):
    ...
    def one_propagate(self, graph, A_feature, B_feature, mess_dropout, test):
        ...
        G_u_list = [None] * (self.num_layers + 1)
        G_b_list = [None] * (self.num_layers + 1)
    ...
    def get_IU_bundle_rep(self,IL_item_feature):

        IU_bundle_feature = torch.matmul(self.bundle_agg_graph_IU,IL_item_feature)
        ...
        BIU_bundle_feature = torch.matmul(self.bundle_agg_graph_BU,IU_bundle_feature)
        return BIU_bundle_feature
    ...
    def propagate(self, test=False):
        ...
        BIU_bundles_feature = self.get_IU_bundle_rep(IL_items_feature)
        IL_bundles_feature = (IL_bundles_feature + BIU_bundles_feature)/self.weight
    ...
    def cal_Gc_loss(self, pos, aug):
        # pos: [batch_size, :, emb_size]
        # aug: [batch_size, :, emb_size]
        pos = pos[:, 0, :]
        aug = aug[:, 0, :]
        c_temp = 0.4 #netease 0.2
    ...
    def forward(self, batch, ED_drop=False):
        ...
        # add CL loss between Gu,GU and Eu 
